...
class apache {
    ...
    exec {
        ...
        "copy-apache-conf":
            command => '/usr/bin/sudo cp /vagrant/private/vagrant/apache.conf /etc/apache2/sites-enabled/apache.conf';

        "copy-apache-ssl-conf":
            command => '/usr/bin/sudo cp /vagrant/private/vagrant/apache_ssl.conf /etc/apache2/sites-enabled/apache_ssl.conf';
...
class mysql {
    exec {
        "mysql-bind-address":
            command => '/usr/bin/sudo cp /vagrant/private/vagrant/bind.cnf /etc/mysql/conf.d/bind.cnf',
...
class php {
    exec {
        ...
        "memcached-bind-address":
            command => '/usr/bin/sudo cp /vagrant/private/vagrant/memcached.conf /etc/memcached.conf';
    }
}
...
class solr {
    exec {
        ...
        "locate":
            ...
            require => Exec['untar'];
        "config":
            command => '/bin/cp /vagrant/private/vagrant/solr.xml /etc/tomcat6/Catalina/localhost/',
            require => Exec['locate'];
        "own":
